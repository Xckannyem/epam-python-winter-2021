...
def is_sorted(num_list: list[int], sort_order: SortOrder) -> bool:
    if not all(isinstance(el, int) for el in num_list) or not isinstance(sort_order, SortOrder):
        raise TypeError
    return num_list == sorted(num_list, reverse=sort_order.value)


def transform(num_list: list[int], sort_order: SortOrder) -> bool:
    if not is_sorted(num_list, sort_order):
        return num_list
    return [i + el for i, el in enumerate(num_list)]


print(transform([15, 14, 11], SortOrder.descending))
